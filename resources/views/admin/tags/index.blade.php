    ...
    <div class="row">
        <div class="col-2 rounded-start" style="background-color: rgb(109, 76, 53)">
            <div class="d-flex justify-content-center" style="margin: 5% 0% 0%">
                <a href="{{route('admin.tags.create',$user)}}" class="btn btn-success">Nueva Etiqueta</a>
            </div>
        </div>
        <div class="col-7" style="background-color: rgb(193, 154, 107)">
            <div class="row" style="margin: 2% 0% 1%">
                <p class="h2">Tabla Etiquetas</p>
            </div>
                        ...
                        <tbody>
                            @foreach ($tags as $index => $tag)
                                <tr>
                                    <th>{{ $index + 1 }}</th>
                                    <td>{{ $tag->name }}</td>
                                    <td><a href="{{route('admin.tags.edit',['user'=>$user,'tag'=>$tag->id])}}" class="btn btn-secondary">Editar</a></td>
                                    <td>
                                        <form action="{{ route('admin.tags.destroy', ['user' => $user,'tag'=> $tag->id]) }}" method="post">
                                            @csrf
                                            @method('delete')
        ...
        <div class="col-3 rounded-end" style="background-color: rgb(109, 76, 53)">
            ...
            <div class="row" style="margin:2% 2% 2%">
                <a href="{{ route('admin.tags', $user) }}" class="btn btn-success">Etiquetas</a>
            </div>
